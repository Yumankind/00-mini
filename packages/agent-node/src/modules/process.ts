 ...
 * `beforeExit`/`exit` listeners actually fire, `emitWarning`, `memoryUsage` (zeros).
 *
...
export interface NodeProcess extends EventEmitter {
  ...
  stdout: WritableSink;
  stderr: WritableSink;
  stdin: { on(): unknown; read(): null; setEncoding(): unknown; resume(): unknown; pause(): unknown; isTTY: boolean };
  cwd(): string;
...
export function createProcess(opts: NodeProcessOptions = {}): NodeProcess {
  ...
  return Object.assign(emitter, {
    ...
    exit: (code?: number): never => {
      throw new ExitSignal(typeof code === "number" ? code : (emitter.exitCode ?? 0));
    },
