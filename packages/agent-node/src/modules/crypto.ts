 ...
 * unless the value is already resolved — EXCEPT for md5 and sha1-of-small-inputs, which are
 * implemented here in JS precisely so the synchronous Node shape keeps working where npm needs it
 * (`npm` itself hashes with sha512 and awaits, so the async road is the one that matters).
 *
...
export class Hash {
  ...
  async digestAsync(encoding?: DigestEncoding): Promise<string | Uint8Array> {
    ...
    if (this.algorithm === "md5") return encodeDigest(md5(body), encoding);
    if (this.algorithm === "sha1") return encodeDigest(sha1(body), encoding);
  ...
  /** Node's shape. Only md5 and sha1 can answer synchronously; the rest name the async road. */
  digest(encoding?: DigestEncoding): string | Uint8Array {
    if (this.algorithm === "md5") return encodeDigest(md5(this.body()), encoding);
    if (this.algorithm === "sha1") return encodeDigest(sha1(this.body()), encoding);
    throw new NodeCompatError(
      "ERR_CRYPTO_ASYNC_ONLY",
      `hash.digest() for ${this.algorithm} is asynchronous here: WebCrypto's subtle.digest returns a promise and no polyfill can unwrap one — call hash.digestAsync() and await it (md5 and sha1 answer synchronously)`,
    );
  }
}
...
export class Hmac {
  ...
  constructor(
    ...
    const name = algorithm.toLowerCase().replace(/-/g, "");
    if (!SUBTLE_NAMES[name]) {
      throw new NodeCompatError(
        "ERR_CRYPTO_UNSUPPORTED_DIGEST",
        `crypto.createHmac('${algorithm}'): WebCrypto's HMAC covers sha1, sha256, sha384 and sha512 only`,
      );
    }
    this.algorithm = name;
  ...
  digest(): never {
    throw new NodeCompatError(
      "ERR_CRYPTO_ASYNC_ONLY",
      "hmac.digest() is asynchronous here: WebCrypto signs with a promise — call hmac.digestAsync() and await it",
    );
  }
}
...
export function cryptoModule(): Record<string, unknown> {
  const api = {
    ...
    pbkdf2Sync: refuse("pbkdf2Sync", "use crypto.webcrypto.subtle.deriveBits with PBKDF2 and await it"),
