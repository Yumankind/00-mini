/**
 * Ported from macaly/almostnode (MIT, 6ab61f31) — 50 cases; see test/compat/LICENSE-almostnode
 *
 ...
 * against is ours — src/modules/core.ts's `eventsModule` (the `events` package).
 *
...
import { eventsModule } from "../../src/modules/core.js";
import { api, assert } from "./common.js";

const events = api(eventsModule());
...
describe('events module (Node.js compat)', () => {
  ...
  describe('edge cases', () => {
    ...
    // SKIP: almostnode's emit() swallows a throwing listener; Node's lets it propagate, and so does the `events` package
    it.skip('should handle errors in listeners', () => {
      const emitter = new EventEmitter();
  ...
  describe('inheritance', () => {
    it('should allow extending EventEmitter', () => {
      ...
      emitter.on('something', (value: any) => {
