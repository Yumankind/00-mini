/**
 * Ported from macaly/almostnode (MIT, 6ab61f31) — the corpus's shared helpers; see
 * test/compat/LICENSE-almostnode
 *
 * Theirs, with two additions: `api()`, because every module in this package is a
 * `Record<string, unknown>` built from a context rather than a static export list, and a header
 * saying so. `skip`/`expectedToFail` are kept as they came even where this corpus does not call
 * them, so the file stays diffable against the original.
 */

/* eslint-disable @typescript-eslint/no-explicit-any */

/**
 * A built module, as the corpus wants to read it: by name, without a cast at every call. `any` and
 * not `Record<string, any>` because the corpus does `new Stream()` and `new KeyObject()`, and a
 * record has no construct signature.
 */
export type Api = any;
export function api(module: unknown): Api {
