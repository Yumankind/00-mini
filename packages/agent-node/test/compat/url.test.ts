/**
 * Ported from macaly/almostnode (MIT, 6ab61f31) — 64 cases; see test/compat/LICENSE-almostnode
 *
 ...
 * against is ours — src/modules/small.ts's `urlModule` (WHATWG URL plus the legacy parse/format/resolve).
 *
...
import { urlModule } from "../../src/modules/small.js";
import { api, assert } from "./common.js";

const url = api(urlModule());
const { parse, format, resolve, URL, URLSearchParams, fileURLToPath, pathToFileURL } = url;
...
describe('url module (Node.js compat)', () => {
  describe('url.parse()', () => {
    ...
    describe('auth parsing', () => {
      ...
      // SKIP: almostnode returns 'user:' for the auth of user@host; Node returns 'user', and so do we
      it.skip('should parse username only', () => {
  ...
  describe('url.format()', () => {
    describe('basic formatting', () => {
      ...
      // SKIP: almostnode's format() returns the href when one is present; Node's ignores it and rebuilds from the parts, and so do we
      it.skip('should use href if provided', () => {
        const formatted = format({
    ...
    describe('query formatting', () => {
      // SKIP: almostnode serialises a STRING query; Node's format() ignores one (only an object is serialised), and so do we
      it.skip('should format query from string', () => {
        const formatted = format({
          protocol: 'http:',
          hostname: 'example.com',
          query: 'foo=bar',
    ...
    describe('host vs hostname', () => {
      // SKIP: almostnode prefers hostname; Node's format() prefers host when both are there, and so do we
      it.skip('should prefer hostname over host', () => {
        const formatted = format({
          protocol: 'http:',
