/**
 * Ported from macaly/almostnode (MIT, 6ab61f31) — 66 cases; see test/compat/LICENSE-almostnode
 *
 ...
 * against is ours — src/modules/crypto.ts's `cryptoModule` (WebCrypto, with md5/sha1/sha256 in JS).
 *
...
import { cryptoModule } from "../../src/modules/crypto.js";
import { Buffer as NodeBuffer } from "../../src/modules/core.js";
import { api, assert } from "./common.js";

const Buffer = api(NodeBuffer);

const crypto = api(cryptoModule());
const {
  randomBytes,
  randomUUID,
  randomInt,
...
describe('crypto module (Node.js compat)', () => {
  ...
  describe('crypto.createHash()', () => {
    ...
    describe('multiple algorithms', () => {
      ...
      // SKIP: sha384 has no synchronous digest here: it needs 64-bit words, so it stays on WebCrypto's promise (digestAsync)
      it.skip('should handle SHA-384', () => {
        const hash = createHash('sha384').update('test').digest('hex');
        assert.strictEqual(hash.length, 96); // SHA-384 = 48 bytes = 96 hex chars
      });

      // SKIP: sha512 has no synchronous digest here: it needs 64-bit words, so it stays on WebCrypto's promise (digestAsync)
      it.skip('should handle SHA-512', () => {
  ...
  describe('crypto.sign() and crypto.verify()', () => {
    ...
    // SKIP: crypto.sign/verify: no key objects here — crypto.webcrypto.subtle.sign is the road
    it.skip('should sign data with secret key', () => {
      const signature = sign('SHA256', testData, secretKey);
      expect(signature).toBeInstanceOf(Uint8Array);
      expect(signature!.length).toBeGreaterThan(0);
    });

    // SKIP: crypto.sign/verify: no key objects here — crypto.webcrypto.subtle.verify is the road
    it.skip('should verify signature with same key', () => {
      const signature = sign('SHA256', testData, secretKey);
      const isValid = verify('SHA256', testData, secretKey, signature!);
      assert.strictEqual(isValid, true);
    });

    // SKIP: crypto.sign/verify: no key objects here
    it.skip('should fail verification with different key', () => {
      const signature = sign('SHA256', testData, secretKey);
    ...
    // SKIP: crypto.sign/verify: no key objects here
    it.skip('should fail verification with different data', () => {
      const signature = sign('SHA256', testData, secretKey);
    ...
    // SKIP: crypto.sign/verify: no key objects here
    it.skip('should produce consistent signatures', () => {
  ...
  describe('crypto.createSign() and crypto.createVerify()', () => {
    ...
    // SKIP: createSign/createVerify refuse by name: signing needs a key object and a padding story WebCrypto spells differently
    it.skip('should sign using streaming API', () => {
    ...
    // SKIP: createSign/createVerify refuse by name
    it.skip('should verify using streaming API', () => {
      const signer = createSign('SHA256');
      signer.update(testData);
      const signature = signer.sign(secretKey);
    ...
    // SKIP: createSign/createVerify refuse by name
    it.skip('should support hex encoding for signature', () => {
      const signer = createSign('SHA256');
      signer.update(testData);
    ...
    // SKIP: createSign/createVerify refuse by name
    it.skip('should support base64 encoding for signature', () => {
      const signer = createSign('SHA256');
      signer.update(testData);
    ...
    // SKIP: createSign/createVerify refuse by name
    it.skip('should support multiple updates', () => {
      const signer = createSign('SHA256');
      signer.update('hello');
      signer.update(' ');
      signer.update('world');
  ...
  describe('crypto.timingSafeEqual()', () => {
    ...
    // SKIP: Node's timingSafeEqual THROWS on a length mismatch rather than returning false, and so does ours
    it.skip('should return false for different length buffers', () => {
      const a = Buffer.from('hello');
  ...
  describe('crypto.KeyObject', () => {
    // SKIP: createSecretKey/KeyObject are absent: there is no Node key object here, only WebCrypto's CryptoKey
    it.skip('should create secret key', () => {
      const key = createSecretKey(Buffer.from('secret'));
      expect(key).toBeInstanceOf(KeyObject);
      assert.strictEqual(key.type, 'secret');
    });

    // SKIP: createSecretKey/KeyObject are absent
    it.skip('should report symmetric key size in bits', () => {
      const key = createSecretKey(Buffer.from('1234567890123456')); // 16 bytes
      assert.strictEqual(key.symmetricKeySize, 128); // 16 * 8 = 128 bits
    });

    // SKIP: createSecretKey/KeyObject are absent
    it.skip('should export key material', () => {
  ...
  describe('crypto.pbkdf2() and crypto.pbkdf2Sync()', () => {
    it('should derive key with callback API', async () => {
      ...
      const derivedKey = await new Promise<Buffer>((resolve, reject) => {
        pbkdf2(password, salt, iterations, keylen, digest, (err: any, key: any) => {
          if (err) reject(err);
          else resolve(key);
        });
      });
    ...
    it('should derive consistent keys', async () => {
      ...
      const key1 = await new Promise<Buffer>((resolve, reject) => {
        pbkdf2(params.password, params.salt, params.iterations, params.keylen, params.digest, (err: any, key: any) => {
          if (err) reject(err);
          else resolve(key);
        });
      });
    ...
    it('should produce different keys with different passwords', async () => {
      ...
      const key1 = await new Promise<Buffer>((resolve, reject) => {
        pbkdf2('password1', params.salt, params.iterations, params.keylen, params.digest, (err: any, key: any) => {
          if (err) reject(err);
          else resolve(key);
        });
      });
      ...
      const key2 = await new Promise<Buffer>((resolve, reject) => {
        pbkdf2('password2', params.salt, params.iterations, params.keylen, params.digest, (err: any, key: any) => {
          if (err) reject(err);
          else resolve(key);
        });
      });
    ...
    it('should produce different keys with different salts', async () => {
      ...
      const key1 = await new Promise<Buffer>((resolve, reject) => {
        pbkdf2(params.password, 'salt1', params.iterations, params.keylen, params.digest, (err: any, key: any) => {
          if (err) reject(err);
          else resolve(key);
        });
      });
      ...
      const key2 = await new Promise<Buffer>((resolve, reject) => {
        pbkdf2(params.password, 'salt2', params.iterations, params.keylen, params.digest, (err: any, key: any) => {
          if (err) reject(err);
          else resolve(key);
        });
      });
    ...
    it('should handle Buffer inputs', async () => {
      ...
      const derivedKey = await new Promise<Buffer>((resolve, reject) => {
        pbkdf2(password, salt, 100, 32, 'sha256', (err: any, key: any) => {
          if (err) reject(err);
          else resolve(key);
        });
      });
    ...
    it('should support different key lengths', async () => {
      ...
      for (const keylen of keylens) {
        const derivedKey = await new Promise<Buffer>((resolve, reject) => {
          pbkdf2('password', 'salt', 100, keylen, 'sha256', (err: any, key: any) => {
            if (err) reject(err);
            else resolve(key);
          });
        });
    ...
    // SKIP: pbkdf2 with sha384/sha512: those two have no synchronous hash here, so no synchronous HMAC and no JS PBKDF2
    it.skip('should support different hash algorithms', async () => {
      const digests = ['sha1', 'sha256', 'sha384', 'sha512'];

      for (const digest of digests) {
        const derivedKey = await new Promise<Buffer>((resolve, reject) => {
          pbkdf2('password', 'salt', 100, 32, digest, (err: any, key: any) => {
            if (err) reject(err);
            else resolve(key);
          });
        });
