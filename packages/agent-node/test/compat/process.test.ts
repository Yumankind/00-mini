/**
 * Ported from macaly/almostnode (MIT, 6ab61f31) — 60 cases; see test/compat/LICENSE-almostnode
 *
 ...
 * against is ours — src/modules/process.ts's `createProcess`.
 *
...
import { createProcess as buildProcess } from "../../src/modules/process.js";
import { api, assert, type Api } from "./common.js";

// Through `api()` because the corpus reads EventEmitter methods off the process and passes
// almostnode-only options (`onExit`) that this package's NodeProcessOptions does not declare.
const createProcess = (options: Api = {}): Api => api(buildProcess(options));
...
describe('process module (Node.js compat)', () => {
  describe('process.env', () => {
    ...
    // SKIP: a tab has no environment: this package's process.env is exactly what the host passed to createProcess, with no invented NODE_ENV/PATH/HOME
    it.skip('should have default environment variables', () => {
      const proc = createProcess();
  ...
  describe('process.platform', () => {
    // SKIP: process.platform is 'browser' on purpose — a script branching on the platform should take a branch it can actually execute
    it.skip('should return a valid platform string', () => {
      const proc = createProcess();
  ...
  describe('process.argv', () => {
    ...
    // SKIP: argv[1] is the entry file, and there is no entry until runMain names one
    it.skip('should have at least 2 elements', () => {
      const proc = createProcess();
  ...
  describe('process.exit()', () => {
    ...
    // SKIP: createProcess({ onExit }) is almostnode's option; exit() here emits 'exit' and throws an ExitSignal the loader catches
    it.skip('should call onExit callback', () => {
      const onExit = vi.fn();
    ...
    // SKIP: same: the exit code arrives as ExitSignal.code, not through an onExit callback
    it.skip('should default to exit code 0', () => {
      const onExit = vi.fn();
      const proc = createProcess({ onExit });
  ...
  describe('process.nextTick()', () => {
    ...
    it('should pass arguments to callback', async () => {
      ...
      proc.nextTick((a: any, b: any, c: any) => {
        result = [a, b, c];
      }, 1, 2, 3);
  ...
  describe('process.memoryUsage()', () => {
    ...
    // SKIP: memoryUsage() is zeros on purpose — a tab cannot see its own resident set and a made-up number is worse
    it.skip('should return positive numbers', () => {
      const proc = createProcess();
      const mem = proc.memoryUsage();
