 ...
 * against is ours — src/modules/core.ts's `pathModule` (path-browserify with `resolve` anchored to the process cwd).
 *
...
import { pathModule } from "../../src/modules/core.js";
import { api, assert, pathTests } from "./common.js";

// The corpus's resolve() cases measure against the REAL cwd, and one of them moves it: our path
// module takes the cwd as a function for exactly that reason, so it is read at every call.
const path = api(pathModule(() => process.cwd()));
const { join, resolve, normalize, dirname, basename, extname, relative, parse, format, isAbsolute, sep, delimiter, posix } = path;
...
describe('path module (Node.js compat)', () => {
  ...
  describe('path.dirname()', () => {
    describe('Node.js official test cases', () => {
      ...
      // SKIP: almostnode expects '/'; Node 24 answers '//' for dirname('//a'), and so do we
      const SKIPPED_INPUTS = new Set(["//a"]);

      dirnameTests.forEach(([input, expected]) => {
        const run = SKIPPED_INPUTS.has(input) ? test.skip : test;
        run(`dirname(${JSON.stringify(input)}) === ${JSON.stringify(expected)}`, () => {
  ...
  describe('path.basename()', () => {
    describe('Node.js official test cases', () => {
      ...
      basenameTests.forEach(([input, ext, expected]) => {
        ...
        // SKIP: almostnode expects ''; Node 24 answers 'b.html' when the suffix IS the whole basename, and so do we
        const run = ext !== undefined && basename(input) === ext ? test.skip : test;
        run(desc, () => {
  ...
  describe('path.parse() and path.format()', () => {
    ...
    describe('parse/format roundtrip', () => {
      // SKIP: almostnode expects the trailing slash back; Node's format() drops it too, and so do we
      const SKIPPED_ROUNDTRIPS = new Set(["/home/user/dir/"]);

      pathTests.parseFormatTests.forEach((input) => {
        const run = SKIPPED_ROUNDTRIPS.has(input) ? test.skip : test;
        run(`parse then format preserves ${JSON.stringify(input)}`, () => {
          const parsed = parse(input);
          const formatted = format(parsed);
